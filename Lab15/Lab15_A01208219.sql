...
SET DATEFORMAT dmy
SELECT Descripcion, e.clave, e.Numero, e.Cantidad
FROM Entregan E, Materiales M
WHERE e.clave = m.Clave AND e.Fecha BETWEEN '01/01/2000' AND '31/12/2000'

SET DATEFORMAT dmy
SELECT e.Numero, p.Denominacion, e.Fecha, e.Cantidad, e.Cantidad/2 as 'Cantidad/2'
...
SELECT RFC FROM Entregan WHERE RFC LIKE '[A-D]A%';
SELECT RFC FROM Entregan WHERE RFC LIKE '[^A-D]%';
SELECT Numero FROM Entregan WHERE Numero LIKE '5__6';
...
SELECT Descripcion, e.clave
FROM Entregan E, Materiales M
WHERE e.clave = m.Clave AND e.Fecha BETWEEN '01/01/2000' AND '31/12/2000'

CREATE VIEW cons4 AS
SELECT Descripcion, e.clave, e.Numero
FROM Entregan E, Materiales M
WHERE e.clave = m.Clave AND e.Fecha BETWEEN '01/01/2000' AND '31/12/2000'
...
SET DATEFORMAT dmy
SELECT p.RFC, sum(e.Cantidad)/count(e.RFC) as Promedio
...
SET DATEFORMAT dmy
SELECT m.Descripcion, sum(e.Cantidad)
FROM Entregan e, Materiales m
WHERE e.Clave=m.Clave
  AND e.Fecha BETWEEN '01/01/2000' and '31/12/2000'
...
SELECT TOP 1 e.Clave, sum(e.Cantidad)
FROM Entregan e
